...
int processLoggedInPlayer( Socket *inSock,
		...
		if( isFertileAge( player ) ) {
            ...
            if( canHaveBaby ) {
                if( ( inCurseStatus.curseLevel == 0 && 
					...
					printf("Player %d is an eligible mother\n", player->id);
    ...
    if( SettingsManager::getIntSetting( "forceAllPlayersEve", 0 ) ) {
        parentChoices.deleteAll();
        forceParentChoices = true;
		printf("All players are forced to spawn as Eve\n");
        }
	...
	if( parentChoices.size() == 0 ) {
	  printf("Possible mothers pool is empty");
	}
    ...
    if( parentChoices.size() == 0 && numOfAge == 0 ) {
		...
		printf("New player is indeed going to spawn as Eve\n");
        }
    ...
    if( newObject.nation == 10 ) {
        newObject.clothing.hat = getObject( 87826 );
	newObject.clothing.tunic = getObject( 87830 );
	newObject.clothing.bottom = getObject( 87834 );
	newObject.clothing.backpack = getObject( 87838 );
    }
    if( newObject.nation == 11 ) {
        newObject.clothing.hat = getObject( 87829 );
        newObject.clothing.tunic = getObject( 87833 );
        newObject.clothing.bottom = getObject( 87837 );
        newObject.clothing.backpack = getObject( 87842 );
    }
    if( newObject.nation == 12 ) {
        newObject.clothing.hat = getObject( 87827 );
        newObject.clothing.tunic = getObject( 87831 );
        newObject.clothing.bottom = getObject( 87835 );
        newObject.clothing.backpack = getObject( 87840 );
    }
    if( newObject.nation == 13 ) {
        newObject.clothing.hat = getObject( 87828 );
        newObject.clothing.tunic = getObject( 87832 );
        newObject.clothing.bottom = getObject( 87836 );
        newObject.clothing.backpack = getObject( 87844 );
    }
    ...
    if( parentChoices.size() > 0 ) {
        ...
        if( newObject.isEve ) {
			...
			printf("Eve spawns next to random existing player\n");
            int parentIndex = 
			...
			printf("Baby spawns to random mother\n");
        ...
        if( ! newObject.isEve ) {
			...
			printf("Mother giving birth to baby\n");
    ...
    if( !placed ) {
		printf("So far new player still not placed\n");
    ...
    if( parent == NULL ) {
        // Eve
		printf("New character has no parents\n");
    ...
    while( !quit ) {
                        ...
                        */
                        ...
                        if( actorUseAllowedByNation && ( distanceUseAllowed 
                            ...
                            char wrongSide = false;
										...
										//ObjectRecord *targetObj;		//unused variable - commented by a862, 09.12.2018
                            ...
                            else if( target != 0 && targetUseAllowedByNation) {
                                ObjectRecord *targetObj = getObject( target );
        ...
        if( playerIndicesToSendLineageAbout.size() > 0 ) {
            ...
            for( int i=0; i<playerIndicesToSendLineageAbout.size(); i++ ) {
                ...
                char *parentID = autoSprintf( "%d", nextPlayer->parentID );
                AppLog::infoF("Sending lineage message: %d %d %d\n", nextPlayer->id, nextPlayer->fatherID, nextPlayer->parentID);
                linWorking.appendElementString( pID );
                linWorking.appendElementString( fatherID );
                linWorking.appendElementString( parentID );
                delete [] pID;
                delete [] fatherID;
                delete [] parentID;
                numAdded++;
